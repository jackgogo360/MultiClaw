...
class ToolBatchExecutor:
    ...
    async def execute(self, calls: Sequence[ToolCallSpec]) -> list[ToolCallOutcome]:
        ...
        while index < len(calls):
            if self.enabled and await self._is_concurrency_eligible(calls[index]):
                ...
                while run_end < len(calls) and await self._is_concurrency_eligible(calls[run_end]):
                    run_end += 1
                await self._execute_concurrent_run(calls[index:run_end], outcomes, start=index)
                index = run_end
                continue

            outcomes[index] = await self._execute_one(calls[index])
            index += 1
    ...
    async def _execute_concurrent_run(
        ...
        *,
        ...
        tasks = [
            asyncio.create_task(self._execute_with_semaphore(semaphore, offset, call))
    ...
    async def _execute_with_semaphore(
        ...
        async with semaphore:
            return index, await self._execute_one(call)

    async def _execute_one(self, call: ToolCallSpec) -> ToolCallOutcome:
        builder = self.registry.get(call.name)
        if builder is None:
        ...
        result = await self.scheduler.run(builder, call.arguments)
        return ToolCallOutcome(
            call_id=call.call_id,
            name=call.name,
            observation=Observation(
