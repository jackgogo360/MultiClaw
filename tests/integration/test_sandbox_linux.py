...
def _require_backend_prerequisites() -> None:
    ...
    if os.environ.get("MULTICLAW_RUN_NATIVE_SANDBOX_TESTS") != "1":
        return
    backend_path = os.environ.get("MULTICLAW_NSJAIL_PATH", "")
    if not backend_path:
        pytest.fail("MULTICLAW_NSJAIL_PATH must point to an executable nsjail binary", pytrace=False)
    backend = Path(backend_path)
    if not backend.is_file() or not os.access(backend, os.X_OK):
        pytest.fail(f"missing executable Linux sandbox backend: {backend}", pytrace=False)
...
async def _run_code(builder: CodeExecToolBuilder, code: str, **kwargs):
    invocation = builder.build(builder.validate({"code": code, **kwargs}))
    return await invocation.execute()


@pytest.fixture(scope="module")
def sandbox_tree(tmp_path_factory: pytest.TempPathFactory) -> Iterator[SandboxTree]:
    root = tmp_path_factory.mktemp("native-linux-sandbox")
    ...
    env_secret_value = "LINUX_NATIVE_SECRET_SENTINEL"
    env_secret_path.write_text(f"SECRET={env_secret_value}\n", encoding="utf-8")
    host_socket_path = outside_dir / "host-only.sock"
...
def sandbox_manager(sandbox_tree: SandboxTree) -> Iterator[SandboxManager]:
    ...
    backend_path = Path(os.environ["MULTICLAW_NSJAIL_PATH"])
    manager = SandboxManager.create(
        settings=SandboxSettings(),
        debug=False,
        workspace_root=sandbox_tree.workspace,
        platform_name="Linux",
        backend_override=NsJailBackend(binary=backend_path),
    )
    try:
        manager.initialize()
        if not manager.finalize_readiness().ready:
            pytest.fail(
                "Linux native sandbox readiness gate failed: " + _manager_readiness_detail(manager),
                pytrace=False,
            )
        yield manager
    finally:
...
@pytest.fixture(scope="module")
def code_exec_builder(
    sandbox_tree: SandboxTree,
    sandbox_manager: SandboxManager,
) -> CodeExecToolBuilder:
    return CodeExecToolBuilder(
        sandbox_tree.workspace,
        sandbox_controller=sandbox_manager,
...
@pytest.mark.asyncio
async def test_linux_sandbox_allows_workspace_write(
    sandbox_tree: SandboxTree,
...
@pytest.mark.asyncio
async def test_linux_sandbox_blocks_write_outside_workspace(
    sandbox_tree: SandboxTree,
    shell_builder: ShellToolBuilder,
) -> None:
    target = sandbox_tree.outside_dir / "outside-write.txt"
    if target.exists():
        target.unlink()

    result = await _run_shell(
        shell_builder,
        "printf 'blocked' > " + shlex.quote(str(target)),
    )
...
@pytest.mark.asyncio
async def test_linux_sandbox_blocks_git_writes(
    sandbox_tree: SandboxTree,
    shell_builder: ShellToolBuilder,
) -> None:
...
@pytest.mark.asyncio
async def test_linux_sandbox_hides_dotenv_reads(
    sandbox_tree: SandboxTree,
    shell_builder: ShellToolBuilder,
) -> None:
    result = await _run_shell(
...
@pytest.mark.asyncio
async def test_linux_sandbox_blocks_parent_listener_network(
    shell_builder: ShellToolBuilder,
    parent_tcp_listener: ParentTcpListener,
) -> None:
    result = await _run_shell(
        shell_builder,
        f"nc -w 1 127.0.0.1 {parent_tcp_listener.port} </dev/null >/dev/null 2>&1",
    )
...
@pytest.mark.asyncio
async def test_linux_code_exec_blocks_child_process_creation(
    code_exec_builder: CodeExecToolBuilder,
) -> None:
...
@pytest.mark.asyncio
async def test_linux_sandbox_hides_parent_home_and_host_socket_paths(
    sandbox_tree: SandboxTree,
    shell_builder: ShellToolBuilder,
) -> None:
    host_home = Path.home()
    ...
    result = await _run_shell(
        shell_builder,
        "test ! -e "
        + shlex.quote(str(host_home))
        + " && test ! -e "
        + shlex.quote(str(sandbox_tree.host_socket_path))
        + " && printf absent",
    )
...
@pytest.mark.asyncio
async def test_linux_timed_out_shell_leaves_no_descendants(
    sandbox_tree: SandboxTree,
    shell_builder: ShellToolBuilder,
) -> None:
